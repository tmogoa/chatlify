...
conn.onmessage = function (e) {
    ...
    switch (res.type) {
        ...
        case "error": {
            ...
            switch (res.message) {
                ...
                case "nsu": {
                    //no selected user
                    Utility.showError(
                        "Please select an account to perform the function"
                    );
                    break;
                }
            }
        ...
        case "scd": {
            //sent chat delivered
            document.getElementById("chat-tick-" + res.chatId).innerHTML =
                "/double-tick/";
            break;
        }
        case "scs": {
        ...
        case "scf": {
            //sent chat fail
            Utility.showError(
                "An error occurred. We could not send your message"
            );
            break;
        }
        case "css": {
...
function changeSelectedUser(userElem) {
    ...
    Utility.main_ajax_with_call_back(
        listUsers,
        "/chats",
        "the_user=" + selectedUser,
        "GET"
    );
}
...
function listUsers(xhttp) {
    document.getElementById("s-username").innerHTML = document
        .getElementById("user-" + selectedUser)
